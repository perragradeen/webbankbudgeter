...
namespace Utilities
{
    ...
    public class WinFormsChecks
    {
        ...
        public delegate void SaveFunctionWithArgs(params object[] args);
        ...
        public static DialogResult SaveCheckWithArgs(bool somethingChanged, SaveFunctionWithArgs saveFuncWithArgs, params object[] args) {
            var saveOr = DialogResult.None;
            if (somethingChanged) {
                saveOr = MessageBox.Show("Läget ej sparat! Spara nu?", "Spara?", MessageBoxButtons.YesNoCancel);//Cancel
                if (saveOr == DialogResult.Yes) {
                    saveFuncWithArgs(args);
                }
            }
