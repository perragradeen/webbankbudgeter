    ...
    class ViewUpdateUi
    {
        internal static void UpdateListViewFromSortedList(ListView showEntriesInThis, SortedList kontoEntries)
        {
            ...
            foreach (KontoEntry kontoEntry in kontoEntries.Values)
            {
                ...
                if (RowsExceedMax(ref rowCounter)) break; //Begränsa antal synliga rader
            }
        }

        #region Container adds
        void AddToListview(ListView list, string[] items)
        {
            ...
            list.Items.Add(new ListViewItem(items));//Overkill? hehe, anal. Trodde jag...nu fick jag ju nytta av det så det så
        }
        internal static void AddToListview(ListView list, KontoEntry entry)
        {
            ...
            (list.Items.Add(new ListViewItem(kontoEntryElements, -1, entry.FontFrontColor, Color.Empty, null))).Tag = entry;//man slipper lite tecken och castningarna o likhetstecknet, iom att detta är en fkn//Overkill? hehe, anal. Trodde jag...nu fick jag ju nytta av det så det så
        }
        #endregion
