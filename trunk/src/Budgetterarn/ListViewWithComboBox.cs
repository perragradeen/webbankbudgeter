...
namespace Budgetterarn {
    public partial class ListViewWithComboBox : KontoEntryListView {
        ...
        private void RowKeyPress(object sender, KeyPressEventArgs e) {
            ...
            if (e.KeyChar == 'a') {//Autocat
                SetAutoCategory();
            }
            if (e.KeyChar == 'd') {//Delete
                DetleteSelectedEntry();
            }
            if (e.KeyChar == 's') {//Several
        ...
        private void SetAutoCategory() {
            ...
            var selItemsCat = selEntry != null && !string.IsNullOrEmpty(selEntry.TypAvKostnad) ? selEntry.TypAvKostnad : null;
            ...
            var autoCatMessage = "Spara autokategori? Alltså att altid välja:" + Environment.NewLine
                                 + selectedCategoryText + Environment.NewLine
                                 + "Varje gång info är:" + Environment.NewLine
                                 + slectedInfoDescription + Environment.NewLine
                                 + "?";
        ...
        internal static void UpdateCategoriesWithAutoCatList(ListViewItemCollection items, string infoToCheck) {
            ...
            foreach (ListViewItem listViewItem in items) {
                ...
                if (!string.IsNullOrEmpty(newKe.TypAvKostnad)) {
                    var autoCatMessage = "Rad har redan en typ av kostnad. Ska den nu valda skrivas över? Alltså att byta från:" + Environment.NewLine
                     + newKe.TypAvKostnad + Environment.NewLine
                     + "Till:" + Environment.NewLine
                     + lookedUpCat + Environment.NewLine
                     + "?" + Environment.NewLine
                     + "För info:" + Environment.NewLine
                     + newKe.Info + Environment.NewLine
                     + Environment.NewLine
                     + "För rad (ihopskriven):"
                     + newKe.KeyForThis;
                ...
                listViewItem.SubItems[ValuesandConsts.TypAvKostnadKolumnnummer].Text =
                    newKe.TypAvKostnad;
            }
            #endregion
        }
...
#if DEBUG
            Budgeter.StatusLabelText = x.ToString();
#endif
        } 
        #endregion

        void DetleteSelectedEntry() {
            if (SelectedIndices.Count < 1 ||
                SelectedItems.Count < 1) // || clickedItem == null)
                return;
        ...
        void PopupComboboxOfCaytegories() {
            PopupComboboxOfCaytegories(null);
        }
        void PopupComboboxOfCaytegories(int? selectedSubColumnItem){
            ...
            for (var i = 0; i < Columns.Count; i++) {
                if (start > position && start < end) {
                    ...
                    Budgeter.StatusLabelText = x + ". Column start: " + start 
                        + ". Column end: " + end
                        + ". Column pos: " + position
                        + ". Column selected: " + columnsel
                        + ". Column date width: " + Columns[0].Width
                        + ". Column typ width: " + Columns[2].Width
                        + ". Column kost width: " + Columns[3].Width
                        ;
#endif
            ...
            if (clickedItem != null) {
                comboBoxCategories.Size = new Size(widestText * 4, clickedItem.Bounds.Bottom - clickedItem.Bounds.Top);//Gamla bredden: end - position
