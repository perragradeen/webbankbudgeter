...
namespace Budgeter.Winforms
{
    ...
    public partial class BudgeterForm : Form
    {
        ...
        private Stack<DoneNavigationAction> navigatedNextActionIsStack = new Stack<DoneNavigationAction>();
        #endregion
        ...
        private bool GetAllEntriesFromExcelFile(string excelFileSavePath, bool clearContentBeforeReadingNewFile)
        {
            ...
            var kontoutdragInfoForLoad = new KontoutdragInfoForLoad
            {
                ...
                excelFileSavePathWithoutFileName = Filerefernces.ExcelFileSavePathWithoutFileName, //Todo: gör en funktion för denna eller refa med en filnamns och sökvägsklass....
                ...
                somethingChanged = _somethingChanged,
            };

            // Ladda från fil
            var entriesLoadedFromDataStore = LoadNSave.LoadEntriesFromFile(kontoutdragInfoForLoad, ref _workerThread);
            ...
            var userCanceled =
                SaveFirstCheck(kontoutdragInfoForLoad, checkforUnsavedChanges, somethingLoadedFromFile,
                    _mainThread, _workerThread);
            ...
            var loadResult = LoadNSave.GetAllEntriesFromExcelFile(kontoutdragInfoForLoad,
                _kontoEntries,
                // _mainThread,
                //ref _workerThread,
                saldon,
            ...
            statusText = "No. rows loaded; " + _kontoEntries.Count + " . Skpped: "
                + loadResult.skippedOrSaved + ". File loaded; "
                + kontoutdragInfoForLoad.filePath;//Visa text för anv. om hur det gick etc.
        ...
        private bool SaveFirstCheck(KontoutdragInfoForLoad kontoutdragInfoForLoad,
        ...
        {//TODO: flytta denna till annan fil, ev. skicka med fkn som delegat
            ...
            if (_newKontoEntries.Count > 0)
            {
                foreach (DictionaryEntry item in _newKontoEntries)
                {
                    ...
                    var foundDoubleInUList = m_newIitemsListEdited.CheckIfKeyExistsInUiControl(newKe.KeyForThis);
        ...
        private void AddNewEntriesToUIListsAndMem(bool autoSave)
        {
            ...
            var changeInfo = UiHelpersDependant.AddNewEntries(_kontoEntries, newEntriesFromUi);
            ...
            var kontoutdragInfoForSave = new KontoutdragInfoForSave
                {
                    ...
                    excelFileSavePathWithoutFileName = Filerefernces.ExcelFileSavePathWithoutFileName,
                    sheetName = _sheetName
                };

            var saveResult = LoadNSave.Save(
                kontoutdragInfoForSave,
                // _mainThread, ref statusText, out _workerThread, 
                _kontoEntries,
                // , _sheetName //, ref _somethingChanged
                // , Filerefernces.ExcelFileSavePathWithoutFileName, Filerefernces._excelFileSaveFileName
                saldon);

            _somethingChanged = saveResult.somethingLoadedOrSaved;
            //somethingChanged = false;//Precis sparat, så här har inget hunnit ändras 

            statusText += "Saving done, saved entries; " + saveResult.skippedOrSaved;//Räkna inte överskriften, den skrivs alltid om
        ...
        private void SetLoginUserEtc()
        {
            if (webBrowser1.Document.Body != null)
            {
                ...
                //Set attrib. Login
                userNameElem.SetAttribute("value", "");
                passElem.SetAttribute("value", "");
